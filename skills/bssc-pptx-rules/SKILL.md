...
pptxgenjs 側ではページ番号も著作権表示も付けない（`slideNumber` オプションも使わない）。出力後、2-3. のフォント是正を実行したあとに、このスキル同梱の `scripts/bssc_footer.py` を実行する。著作権表示の有無は 0. の確認に対するユーザーの回答どおりに**必ず**指定する（指定しないとエラーで止まる）。

```bash
python3 <このスキルのディレクトリ>/scripts/bssc_footer.py output.pptx --copyright      # 掲載する
python3 <このスキルのディレクトリ>/scripts/bssc_footer.py output.pptx --no-copyright   # 掲載しない
```

スクリプトの処理内容：

1. `firstSlideNum="0"` を設定する。
2. 全レイアウトの `BSSC_PageNumber`（と `--copyright` 時は `BSSC_Copyright`）を置き直す（再実行しても重複しない＝冪等）。
3. 各スライド上の旧フッター（固定数字の箱、`sldNum` プレースホルダ、右下の数字のみのテキスト、直書きの `© BSSC Co., Ltd.`）を削除し、二重表示を防ぐ。
4. 表紙に `showMasterSp="0"` を設定し、`--copyright` 時は表紙にだけ著作権表示を直接置く。表紙のレイアウトにロゴ等の装飾図形がある場合は「注意」を表示するので、その場合は表紙の見た目を目視確認し、必要な装飾は表紙スライド上に直接配置する。

### 3-4. 既存 PPTX の編集時

- スライドの追加・削除・並べ替えを行った場合も、最後に必ず `bssc_footer.py` を実行する（固定数字の番号や直書きの著作権表示がある既存資料は、このときレイアウト配置方式に置き換わる）。
...
> ページ番号は自動連動です。PowerPoint でスライドを追加・削除・並べ替えしても番号は自動で振り直され、追加したスライドにも著作権表示が自動で入ります（［スライドのサイズ］の「スライド開始番号」は 0 のまま変更しないでください。表紙を差し替える場合は、表紙のフッターは自動では入らないのでご注意ください）。
...
### 4-1. 配置方式（追加スライドへの自動掲載）

- 著作権表示はスライドごとに直書きせず、**全スライドレイアウトに `BSSC_Copyright` として 1 つ置く**。これにより、PowerPoint でスライドを追加しても著作権表示が自動で入る。
- 表紙はページ番号を消すためにレイアウトの図形を非表示（`showMasterSp="0"`）にしているので、**表紙にだけ**同じ位置・書式の著作権表示を直接置く。
- いずれも `bssc_footer.py --copyright` が行う。位置は x=7.45in, y=6.95in, w=2.50in, h=0.35in、右揃え・下揃え（右端 9.95in とページ番号左端 10.10in の間隔 0.15in）。
- ユーザーが位置を個別指定した場合は、スクリプトの `CR_X` 等を変更して同じ方式で配置する。
...
- [ ] 著作権表示がある場合、全レイアウトに `BSSC_Copyright` が 1 つずつあり、表紙にも直接 1 つある（表紙以外のスライドに直書きの重複がない）
- [ ] 著作権表示なしの場合、レイアウト・スライドのどこにも `© BSSC Co., Ltd.` が残っていない
...
for m in prs.slide_masters:
    for l in m.slide_layouts:
        ...
        cr = sum(s.name == "BSSC_Copyright" for s in l.shapes)
        print("レイアウト", l.name, "番号", "OK" if ok else "NG", "著作権", cr)  # 掲載時 1、非掲載時 0
for i, s in enumerate(prs.slides, start=1):
    ...
    # 表紙のみ「非表示」、固定数字は空、著作権は表紙のみ 1 件（掲載時）/ 全スライド 0 件（非掲載時）
```
